    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/55 p-4">
      <div className="w-full max-w-2xl rounded-xl bg-white p-6 shadow-2xl">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div>
            <label className="mb-1 block text-sm font-medium text-slate-700">任务说明</label>
            <textarea
              value={formData.description}
              onChange={(event) => setFormData({ ...formData, description: event.target.value })}
              className="w-full rounded-md border border-slate-300 px-3 py-2 focus:border-blue-500 focus:outline-none focus:ring-2 focus:ring-blue-100"
              rows={3}
              placeholder="补充场景、目标区域、镜头说明等"
            />
          </div>
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              ...
              <select
                ...
                onChange={(event) => setFormData({ ...formData, modelType: event.target.value })}
            ...
            <div>
              ...
              <input
                ...
                onChange={(event) => setFormData({ ...formData, epochs: Number(event.target.value) || 10 })}
                className="w-full rounded-md border border-slate-300 px-3 py-2"
              />
            </div>
            <div>
              ...
              <input
                ...
                onChange={(event) => setFormData({ ...formData, batchSize: Number(event.target.value) || 4 })}
                className="w-full rounded-md border border-slate-300 px-3 py-2"
              />
            </div>
            <div>
              ...
              <select
                ...
                onChange={(event) => setFormData({ ...formData, imageSize: Number(event.target.value) })}
                className="w-full rounded-md border border-slate-300 px-3 py-2"
              >
