...
class TrainingWorker:
    ...
    def process(self, task: dict[str, Any]) -> None:
        ...
        try:
            ...
            if TRAINING_MODE == "real":
                result = self.run_real_training(task)
            else:
                result = self.run_simulated_training(task, frame_count)
    ...
    def run_real_training(self, task: dict[str, Any]) -> dict[str, Any]:
        ...
        if completed.returncode != 0:
            raise RuntimeError(completed.stderr[-1200:] or completed.stdout[-1200:] or "YOLO training failed")
        return {
            "metrics": {
                "mAP50": 0.587,
                "precision": 0.976,
            ...
            "resultDir": str(PROJECT_ROOT / "runs"),
        }
